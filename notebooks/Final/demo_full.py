...
if option == "Home":
    ...
    with col1:
        st.title("Stock Market Prediction and Forecasting Using Machine Learning")
    ...
    st.write('''The successful prediction of a stock's future price could yield significant profit.''')
    st.header("What is the use of stock prediction?")
    ...
    st.write('''The entire idea of predicting stock prices is to gain significant profits.''') 
    st.write('''Predicting how the stock market will perform is a hard task to do''')
    ...
    if submit:
        ...
        st.subheader("Forecasting future prices using the trained model")
        fig2 = plt.figure(figsize = (12,6))
...
# ----------------------------------------------
    ...
    st.title("Stock Trend Prediction RNN")
    user_input = st.text_input("Enter Stock Ticker", "AAPL")
    df = pdr.get_data_yahoo(user_input, start, end)
